...
% Biblioteca config, fitxers config, variables entorn, logs, 
...
\label{sec:seguretat}
%Obtenció token navegador, crida dades usuari, usuari activat / desactivat o eliminat, axios interceptors
...
\begin{enumerate}
  ...
  \end{figure}
  \item Per últim, tant les dades de l'usuari com el \textit{token} generat s'envien com a respota al client, que guarda el \textit{token} al \textit{local storage} del navegador per tal de mantenir la sessió iniciada.
\end{enumerate}
